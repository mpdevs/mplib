...
-- predict
DROP TABLE IF EXISTS competitive_item_predict_stage_1;
CREATE TABLE competitive_item_predict_stage_1(
    customer_item_id BIGINT,
    target_item_id BIGINT,
    similarity STRING,
    score STRING,
...
INSERT INTO competitive_item_predict_stage_1
SELECT customer_item_id,
       target_item_id,
       similarity,
       '0' AS score,
       1623 AS category_id
FROM competitive_filtering_stage_2;

DROP TABLE IF EXISTS competitive_item_predict_stage_2;
CREATE TABLE competitive_item_predict_stage_2 AS
SELECT
    a.customer_item_id,
    a.target_item_id,
    a.category_id,
    ...
    tn.attrvalue AS target_item_attr,
    a.score
FROM competitive_item_predict_stage_1 AS a
JOIN mpintranet.attrname_export AS cn
ON a.customer_item_id = cn.itemid
JOIN mpintranet.attrname_export AS tn
ON a.target_item_id = tn.itemid
